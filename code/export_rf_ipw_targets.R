...
v_arg <- mat[as.character(find_id("^arginine$")), ]
v_orn <- mat[as.character(find_id("^ornithine$")), ]
v_cit <- mat[as.character(find_id("^citrulline$")), ]
if(!is.null(v_arg)) mat <- rbind(mat, "GABR" = log2(unlog(v_arg)/(unlog(v_orn)+unlog(v_cit))))

v_lac <- mat[as.character(find_id("^lactate$")), ]
v_pyr <- mat[as.character(find_id("^pyruvate$")), ]
v_citr <- mat[as.character(find_id("^citrate$")), ]
...
set.seed(42)
m_meta <- m_meta %>% mutate(Group_Sub = case_when(PTSD_status_binary=="Control"~"Control", four_subtypes=="Depressive Symptom Subtype"~"Depressive", four_subtypes=="Impaired Cognitive Function"~"Cognitive", four_subtypes=="Subthreshold/Mild PTSD Subtype"~"MildPTSD", TRUE~NA_character_)) %>% filter(!is.na(Group_Sub))
mat <- mat[, m_meta$Metabolon_ID]

for(tgt in c("Depressive", "Cognitive", "MildPTSD")) {
  sub_m <- m_meta[m_meta$Group_Sub %in% c("Control", tgt), ]
  ...
  fit <- eBayes(contrasts.fit(lmFit(sub_mat, dsn, weights=sub_m$w), makeContrasts(paste0(tgt,"-Control"), levels=dsn)), robust=TRUE, trend=TRUE)
  ...
  write.csv(res_out, file.path(out_dir, paste0("RF_IPW_", tgt, "_vs_Control.csv")), row.names=FALSE)
}
