...
if (nrow(common_metabs) == 0) {
  cat("No common metabolites found with P < 0.01 in >= 2 categories.\n")
  q()
}
...
# Create the HTML doc
doc_file <- file.path(dirname(out_dir), "result_FINAL_PRODUCTION", "V12_Common_Metabolites_Heatmap_P0.01.doc")
sink(doc_file)
...
cat("</style>\n")
cat("</head><body>\n")
cat("<h2>Common Metabolites Heatmap (P < 0.01 in >= 2 categories)</h2>\n")
