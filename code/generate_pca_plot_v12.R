...
cat("Generating V12 Focus Panel PCA plot...\n")
...
panel_file <- file.path(working_dir, "V12", "V12_panel_features.txt")
...
v12_features <- as.character(read.table(panel_file, header=FALSE)$V1)
...
# Extract metabolomics matrix, but ONLY for V12 features
# Ensure we only select features that exist in the column names
valid_v12_features <- intersect(v12_features, colnames(plot_data))
cat(sprintf("Found %d out of %d V12 panel features in the dataset.\n", length(valid_v12_features), length(v12_features)))

metab_matrix <- as.matrix(plot_data %>% select(all_of(valid_v12_features)))
# Ensure numeric
...
p <- ggplot(pca_df, aes(x = PC1, y = PC2, color = Group)) +
  ...
  theme_minimal() +
  labs(
    title = "PCA of Metabolomics Data (V12 Focus Panel) by Subtype",
...
# Save plot
plot_file <- file.path(out_dir, "V12_Metabolites_PCA_Subtypes_Focused.png")
