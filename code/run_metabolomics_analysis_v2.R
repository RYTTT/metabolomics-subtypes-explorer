...
output_dir <- file.path(working_dir, "Meta subtype  Antigravity", "result")
...
# --- 2a. Filter Lipids ---
cat("\n--- Filtering Lipids ---\n")
if ("SUPER_PATHWAY" %in% colnames(annotation)) {
  # Identify Lipid metabolites
  lipid_rows <- annotation %>% 
    filter(SUPER_PATHWAY == "Lipid")
  
  lipid_ids <- lipid_rows$CHEM_ID
  
  # Check overlap with matrix rows
  # mat_final rownames are CHEM_IDs (as characters or typically matched)
  # Ensure type match
  rows_to_remove <- rownames(mat_final) %in% as.character(lipid_ids)
  
  cat(paste("Total metabolites before lipid filter:", nrow(mat_final), "\n"))
  cat(paste("Removing", sum(rows_to_remove), "lipid metabolites.\n"))
  
  mat_final <- mat_final[!rows_to_remove, ]
  cat(paste("Remaining metabolites:", nrow(mat_final), "\n"))
} else {
  cat("Warning: SUPER_PATHWAY column not found. Skipping lipid filter.\n")
}

# --- 2b. Filter Unknowns (X-) ---
cat("\n--- Filtering Unknown Metabolites (X-*) ---\n")
if ("CHEMICAL_NAME" %in% colnames(annotation)) {
  # Identify X- metabolites
  unknown_rows <- annotation %>% 
    filter(grepl("^X-", CHEMICAL_NAME))
  
  unknown_ids <- unknown_rows$CHEM_ID
  
  rows_to_remove_x <- rownames(mat_final) %in% as.character(unknown_ids)
  
  cat(paste("Removing", sum(rows_to_remove_x), "unknown metabolites (X-).\n"))
  
  mat_final <- mat_final[!rows_to_remove_x, ]
  cat(paste("Remaining metabolites:", nrow(mat_final), "\n"))
}
...
# --- 2d. Filter Low Standard Deviation ---
cat("\n--- Filtering Low SD Metabolites ---\n")
# Calculate SD for each metabolite (row)
row_sds <- apply(mat_final, 1, sd, na.rm = TRUE)

# Determine cutoff (remove bottom 30%)
sd_quantile <- 0.30
sd_cutoff <- quantile(row_sds, probs = sd_quantile, na.rm = TRUE)

keep_high_sd <- row_sds > sd_cutoff

cat(paste("SD Cutoff (30%):", round(sd_cutoff, 4), "\n"))
cat(paste("Total metabolites before SD filter:", nrow(mat_final), "\n"))
cat(paste("Removing", sum(!keep_high_sd), "metabolites with low SD.\n"))

mat_final <- mat_final[keep_high_sd, ]
cat(paste("Final metabolite count:", nrow(mat_final), "\n"))
