...
out_dir <- file.path(working_dir, "Meta subtype  Antigravity", "result_FINAL_PRODUCTION", "IPW_Six_Comparisons")
...
idx_complete <- complete.cases(m_meta[, c("Age", "Gender", "BMI", "Cohort", "four_subtypes")])
m_meta <- m_meta[idx_complete,]
mat <- mat[, idx_complete]

m_meta <- m_meta %>% mutate(Group_Sub = case_when(
...
subtypes <- c("Depressive", "Cognitive", "MildPTSD", "SeverePTSD")
combos <- combn(subtypes, 2)
...
for(i in 1:ncol(combos)) {
  ...
  sub_m <- m_meta %>% filter(Group_Sub %in% c(g1, g2))
  sub_mat <- mat[, sub_m$Metabolon_ID]
  sub_m$IsCase <- ifelse(sub_m$Group_Sub == g1, 1, 0)
  ...
  q_low <- quantile(sub_m$SW, 0.05)
  q_high <- quantile(sub_m$SW, 0.95)
  ...
  # Safe contrasts
  levs <- levels(Grp)
  cname <- paste0(levs[1], "-", levs[2])
  
  fit <- eBayes(contrasts.fit(lmFit(sub_mat, dsn, weights=sub_m$Trunc_Weight), makeContrasts(cname, levels=dsn)), robust=TRUE, trend=TRUE)
  res <- topTable(fit, coef=1, n=Inf, sort.by="P")
  
  sig_01 <- sum(res$P.Value < 0.01)
  sig_fdr <- sum(res$adj.P.Val < 0.05)
  
  res_row <- data.frame(Comparison = paste0(g1, " vs ", g2), Actual_Internal_Contrast = cname, P_01_Nodes = sig_01, FDR_05_Nodes = sig_fdr)
  results_out <- rbind(results_out, res_row)
  ...
  write.csv(res_out, file.path(out_dir, paste0("Pairwise_", g1, "_vs_", g2, ".csv")), row.names=FALSE)
}
