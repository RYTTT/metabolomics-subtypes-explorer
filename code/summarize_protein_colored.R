...
file_path <- "/Users/ruotingyang/Desktop/Projects/Meta subtype/Meta subtype  Antigravity/resource/Protein subtype analysis with directionality_updated 4.29.2026.xlsx"
out_dir <- "/Users/ruotingyang/Desktop/Projects/Meta subtype/Meta subtype  Antigravity/result_FINAL_PRODUCTION"
doc_file <- file.path(out_dir, "Protein_DE_summary_Colored.doc")
...
cat("<h2>Differential Protein Expression Summary (Colored)</h2>\n")
cat("<p>Grouped by Protein Category. Showing significant proteins (P < 0.05). Colors: <span style='color:red'><b>red</b></span> = upregulated, <span style='color:blue'><b>blue</b></span> = downregulated.</p>\n")
...
for (s in sheets) {
  ...
  if(nrow(df_sig) == 0) {
    ...
    dir_col <- grep("Directionality|Directionary", colnames(df_sig), ignore.case = TRUE, value = TRUE)
    ...
    df_sig$formatted <- sapply(1:nrow(df_sig), function(i) {
      ...
      color <- "black"
      if (length(dir_col) > 0) {
        dir_val <- tolower(as.character(df_sig[[dir_col[1]]][i]))
        if (grepl("up", dir_val)) color <- "red"
        else if (grepl("down", dir_val)) color <- "blue"
      }
      
      sprintf("<span style='color:%s'>%s%s</span>", color, df_sig[["Protein Name"]][i], stars)
    })
    
    # Handle missing categories
    if (!"Protein Category" %in% colnames(df_sig)) {
      df_sig[["Protein Category"]] <- "Other/Unspecified"
    } else {
