...
doc_file <- file.path(dirname(out_dir), "result_FINAL_PRODUCTION", "V12_DE_summary_Original.doc")
sink(doc_file)

cat("<html><head><meta charset='utf-8'></head><body>\n")
cat("<h2>DE Metabolites Summary (v12 panel) - Standard Pooled Limma (Yesterday's Method)</h2>\n")
cat("<p>Based on the new control set (<code>result_V12</code> folder). Colors: <span style='color:red'><b>red</b></span> = upregulated (logFC > 0), <span style='color:blue'><b>blue</b></span> = downregulated (logFC < 0).</p>\n")
